...
void ingresarProducto() {
    ...
    char (*ptrNombre)[MAX_NOMBRE] = nombres;
    int *ptrCantidad = cantidades;
    int *ptrTiempo = tiemposFabricacion;
    int *ptrRecurso = recursosNecesarios;

    printf("Nombre del producto: ");
    fgets(*(ptrNombre + totalProductos), MAX_NOMBRE, stdin);
    (*ptrNombre + totalProductos)[strcspn(*(ptrNombre + totalProductos), "\n")] = '\0';

    printf("Cantidad demandada: ");
    *(ptrCantidad + totalProductos) = leerEnteroSeguro();

    printf("Tiempo de fabricacion por unidad (minutos): ");
    *(ptrTiempo + totalProductos) = leerEnteroSeguro();

    printf("Recursos necesarios por unidad: ");
    *(ptrRecurso + totalProductos) = leerEnteroSeguro();
